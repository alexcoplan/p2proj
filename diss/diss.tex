...
just given by $H(p,\widetilde{p})$:
$$ - \log{ p(x_t | x_{t-1}, \ldots, x_1) } $$
...
this information, obtained from the \emph{Riemenschneider} edition of the
chorale harmonisations \todo (cite?), in the corpus preparation script. 

Following the process for hyperparamter optimisation described in
Section~\ref{sec:mvs-hyperparams}, the best multiple viewpoint system (MVS) was
found to have the following hyperpameters:
\begin{itemize}
...
The best-performing MVS achieved a cross-entropy of $2.825\ \mathrm{bits}$ on
the test set, while the best-performing RNN achieved $2.328\ \mathrm{bits}$. For
the MVS, this can be decomposed into $1.881\ \mathrm{bits}$ for pitch, $0.832\
\mathrm{bits}$ for duration, and $0.113\ \mathrm{bits}$ for rest.

These results show that the RNN significantly outperforms the MVS in predictive
ability. However, one cannot immediately conclude from this that the RNN will
generate more successful outputs when sampled from. This remains to be
demonstrated.

\subsection{Analysis}

While a single figure measuring the overall performance of each model is useful,
it is not particularly insightful. In this section, we take a more fine-grained
approach by looking at the \emph{entropy profile} of various sequences with
respect to each model. In each profile, we show the cross-entropy: a measure of
how poorly the model predicted each event, and the \emph{distribution entropy}:
a measure of overall uncertainty in the prediction at each point.

\begin{figure}[H]
\centering

\subfloat[RNN Entropy Profile] {
  \label{subfig:rnn-aus-meines}
  \includegraphics[width=220pt]{figs/rnn_aus_meines_profile.pdf}
}
\subfloat[MVS Entropy Profile] {
  \label{subfig:mvs-aus-meines}
  \includegraphics[width=220pt]{figs/mvs_aus_meines_profile.pdf}
}

\caption{Entropy profiles of models predicting Chorale Melody \emph{Aus Meines
Herzens Grunde}}
...
\subfloat[RNN Entropy Profile] {
  ...
  \includegraphics[width=220pt]{figs/rnn_danket_dem_herrn.pdf}
}
\subfloat[MVS Entropy Profile] {
  \label{subfig:mvs-danket-dem}
  \includegraphics[width=220pt]{figs/mvs_danket_dem_herrn.pdf}
}

\caption{Entropy profiles of models predicting Chorale Melody \emph{Danket dem
Herrn heut und allzeit}}
...
\subfloat[RNN Entropy Profile] {
  ...
  \includegraphics[width=220pt]{figs/rnn_path_eg.pdf}
}
\subfloat[MVS Entropy Profile] {
  \label{subfig:mvs-path}
  \includegraphics[width=220pt]{figs/mvs_path_eg.pdf}
}
...
web application. The survey stored three pools of samples: one for the MVS,
another for the RNN, and a third for genuine chorale melodies. Initially, users
were asked to give a self-assessment of their musical ability. Upon a user
starting the survey, two samples would be selected from each pool at random.
After shuffling these samples, they would be presented to the user, at which
point they were asked, for each sample:
\begin{enumerate}[label=\arabic*., itemsep=0mm]
  \item To state whether the sample was composed by a human (i.e.\ a genuine
    chorale) or a computer, as well as giving a confidence rating (from 0 to 4,
    expressed as English labels) for this judgement.
  \item To rate, on a Likert scale, the \emph{coherency} of the sample as a
    musical composition.
\end{enumerate}

The decision to include a confidence rating was motivated by a discussion with
an author of some recent related research \cite{liang2016bachbot}.
...
\todo Include distribution of experiences received?
...
indicate ``human-composed'' and $-1$ to indicate ``machine-composed''.
