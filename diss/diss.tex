...
\todo cite bengio paper. A RNN is an end-to-end sequence learning tool which
...
\begin{displayquote}
  ...
  generalize to sequence lengths not seen during training, nor share statistical
...
through time} (BPTT), appplies the standard backpropagation algorithm to the
unfolded computational graph for our RNN. That is, we calculate the derivative
of the cost function with repsect to the weights in our network \emph{at each
...
parallellised, since the computation at timestep $t$ depends on
...
this can be desirable, granting the network much expressivity, training such
...
\subsubsection{Long-term dependenices}
...
and, supposing that $W$ has an eigendecomposition of the form:
...
experience an exponential slowup in learning such dependencies. Eventually,
...
\cite{hochreiter1997long}. Despite the superifical complexity of this
...
minor \emph{linear}, \emph{pointwise} interactions, protected by strucutres
known as gates. 
...
Let subscripts denote timesteps and superscripts denote layers so that $h_t^l
\in \mathbb{R}^n$ denotes the state at time $t$ in layer $l$. In order to
...
the network. Let $h_t^0 = E(x_t)$ denote the input vector at time $t$ and take
$h_t^L$ as the output vector at time $t$.
...
\begin{enumerate}[label=\arabic*., itemsep=0mm]
  ...
  \item \textbf{Forget} selectively: occassionaly, previously-useful memories
...
unit} (GRU). Generally, LSTM variants have only been shown to achive marginal
...
chorales and viewpoints for the chorale dataset. 
...
class diagram outlining the ``prediction stack'': the generic viewpoint
implementation. A design goal of the implementation was to achieve a symmetry
and tight correspondence between the \texttt{C++} type system and the formalism
of multiple viewpoints detailed in Section~\ref{sec:mvs-formalism}.

\todo Give high-level overview of the implementation including UML class diagram
of all classes implemented.

\todo Then give bullet-point description of the function of each component.

\todo Then we can really dive into the chronological-ish (primarily, bottom-up)
section-by-section appraoch that follows.
...
implementation adds a \texttt{forget\_bias} of 1 to the biases of the forget gate
because it helps towards the beginning of training.
