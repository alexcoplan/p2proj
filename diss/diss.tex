...
the trie, the resulting node initialised to have a count of 1. Note that $()$,
...
Figure~\ref{fig:bad-ppm-trie} illustrates the situation.
...
The first problem is the ecsape node labelled $\epsilon$. According to
...
\frac{6}{7} \neq 1. $$
The observation to make here is that if \emph{all} the symbols in our alphabet
have a non-zero count for any given context, then the escape probability is
...
$\mathbb{P}(\alpha|\alpha) = 1/4$ and $\mathbb{P}(\beta|\alpha) = 1/2$. By
(\ref{eq:ppm-general}) we find $\mathbb{P}(\gamma|\alpha) = 1/4 \cdot
\mathbb{P}(\gamma)$, but as we have already calculated, $\mathbb{P}(\gamma) <
1$, so this is not normalised. The problem here is that the escape mass for the
node labelled $\alpha\epsilon$ is distributed among all the symbols at layer $h
= 1$ in the trie, when really it should be distributed just over $\gamma$.
...
The key to \emph{exclusion} in this algorithm is the \textit{Dead} set that is
  ...
  \begin{algorithmic}[1]
    ...
    $\textit{Dead} \subseteq [\tau]$}
      ...
      \If{$\textit{Novel} = \varnothing$}
        \State \Return $C(e_i^k :: e') / s$ \Comment{No possible need for ecsape}
      \EndIf
      \If{$e' \in \textit{Known}$}
...
of the pitch of the notes: thus, it is likely to be fruitful to model duraiton
...
\emph{Viewpoints} allow us to model
music from these different points of view: namely, we can
model the individual $\tau_i$, as well as abstract interpretations and
combinations thereof. We now proceed to set up the formalism around viewpoints
as per Conklin and Witten \cite{conklin1995viewpoints}.
...
surface event stream $e_1^k \in \zeta^*$, if such an element exists.
...
Now conisder a viewpoint over a type $\tau$ derived from some surface type
...
this process as \emph{reification}, specified by a partial function $\Phi^{-1} :
