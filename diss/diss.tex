...
these basic types individaully.

\texttt{BasicViewpoint<T>} is then just a simple wrapper around
\texttt{ContextModel<T>} that conforms to \texttt{Predictor<ChoraleEvent, T>}.
...
We make use of two abstract interpreations of the pitch surface to improve
...
To predcit a distribution over either of the constitutent types of a linked
viewpoint, one simply sums out the joint distribution over the other type: a
viewpoint $\tau_1 \rightarrow \tau_2$ predicts a distribtuion over $[\tau_1]
\times [\tau_2]$ and then sums over $[\tau_1]$ to obtain a distribtuion over
$[\tau_2]$.
...
implemetnation. To implement $\emph{seqint} \rightarrow \emph{duration}$, we
would need to implement a specific linked viewpoint for this combination of
types that performs the lifintg and reification for \emph{seqint}.
...
Template metaprogramming was then utilised to compute the surface type of any
...
\begin{enumerate}[label=(\arabic*), itemsep=0mm]
  ...
  \item The inter- and intra-layer bias paramters for distribution combination.
...
\ref{itm:bias-params}, and repeat until convergence. While this techique cannot
