...
that (\ref{eq:ppm-general}) gives rise to complete probability distributions.
This is in fact not the case, as illustrated by \todo.

\todo Figure illustrating why simple formulation is not complete.

There are two typical solutions for this lack of normalisation in PPM: in
earlier work, authors often simply compute an improper distribution and
later normalise it (e.g.\ \cite{conklin1990prediction}). A more sophisticated
solution which reasons about the possible $h$-grams that can be escaped to is
known as PPM with \emph{exclusion} \cite{pearce2004improved}, detailed in
...
\Call{ppm-a}{$e_1^k$, $1$, $e'$, $\varnothing$}.

\begin{algorithm}[H]
  \caption{PPM A with exclusion}
  \label{alg:ppm-a}
  \begin{algorithmic}[1]
    \Function{ppm-a}{$e_1^k$, $i_{\mathrm{begin}}$, $e'$,
    \textit{Dead}}
      ...
      \If{$\textit{Novel} = \varnothing$}
        \State \Return $C(e_i^k :: e') / \sum_{e'' \in \textit{Alive}}
        C(e_i^k :: e'')$ \Comment{No possible need for ecsape}
      \EndIf
      \If{$e' \in \textit{Known}$}
      \State \Return $C(e_i^k::e') / (1 + \sum_{e'' \in \textit{Alive}}
      C(e_i^k :: e'') )$ \Comment{Matched $e_i^k::e'$}
      \EndIf
      \State $p_\epsilon \gets 1 / (1 + \sum_{e'' \in Alive} C(e_i^k :: e''))$
      \State \Return $p_\epsilon\ \cdot\ $\Call{ppm-a}{$e_1^k$, $i+1$, $e'$,
      $Dead \cup Known$} \Comment{Escape recursively}
    \EndFunction
  \end{algorithmic}
\end{algorithm}
