...
There are in fact many LSTM variants: we use the architecture of Zaremba et al.\
\cite{zaremba2014recurrent}. Before introducing LSTMs in detail, we introduce
the notion of deep recurrent networks.
...
both Python and \texttt{C++}, as well as its human-readable nature, so as to
simplify the debugging process. This JSON format was used not only for the
corpus, but also for musical I/O of both models.
...
In addition to extracting melodies from the \texttt{music21} chorale corpus and
encoding them in our JSON format, the corpus preparation script performs
analysis on the corpus to determine the domains of musical types in our corpus.
That is, the script computes the sets of pitches, durations, intervals, etc.\
that occur in the chorales, and stores these as metadata along with the corpus. 

This informed the representation of musical types as \texttt{C++} classes (see
Section~\ref{sec:cpp-event-rep}). Figures~\ref{fig:key-dist} and
\ref{fig:range-dist} exemplify the analysis performed on the corpus. By
preprocessing the corpus, we also factor out this work from the point at which
data is ingested into each model.
...
Before implementing the RNN itself, I implemented a module to ingest
...
separately: for text, $10\%$ of the text is kept aside as test data. I chose to
delegate the train/test split of musical data to the corpus preparation stage
since this needs particular care. The split is therefore specified in the
\texttt{JSON} file. 
