...
\section{Choice of Corpus and Representation}
...
context models, which can be parameterised over arbitrary types using \verb!C++!
templates. The distributions predicted by these context models can be combined
using the entropy-weighted schemes described in Section~\ref{sec:vp-comb}.
...
eliminate much of the need for dynamic memory allocation, a cause of many
memory-related issues.

Although inheritance polymorphism is used for one class hierarchy (see Figure\
\ref{fig:uml-prediction}), templates are preferred and used widely through the
implementation.
...
specialised to implement the components specific to our chosen corpus. 
...
\begin{itemize}
  \item \texttt{TrieNode<b>} is an implementation of the trie data structure
    with branching factor \texttt{b}.
