...
\tikzset{every text node part/.style={align=center}}
%% nodes for t = 0
\node[n] (ctx) {Context\\$(e_1,\ldots,e_k)$};
\node (model) [right = 1.0cm of ctx] {\textbf{Generative}\\\textbf{Model}};
\node[n] (MVS) [above= 0.3cm of model] {MVS};
\node[n] (RNN) [below= 0.3cm of model] {RNN};
\node[n] (dist) [right= 1.0cm of model] {Prediction\\$p(e' | e_1, \ldots, e_k)$ };
\node[n] (sample) [right= 0.8cm of dist] {Sample\\$e_{k+1}$};
...
\path (sample) edge [dashed,out=90,in=90,anchor=north,looseness=0.45] (ctx);
\end{tikzpicture}
...
%% nodes for t = 0
\node[n] (root) {$() : 6$};
\node[n] (b) [below of=root] {$\beta : 2$};
\node[n] (a) [left =2.0cm of b] {$\alpha : 3$};
\node[n] (ag) [below of=a,gray,dashed] {$\alpha\gamma : 0$};
\node[n] (ab) [left of=ag] {$\alpha\beta : 2$};
\node[n] (aa) [left of=ab] {$\alpha\alpha : 1$};
\node[n] (ae) [right of=ag] {$\alpha\epsilon : 1$};
\node[n] (bg) [below of=b] {$\beta\gamma : 1$};
\node[n] (g) [right of=b] {$\gamma : 1$};
\node[n] (ga) [below of=g] {$\gamma\alpha : 1$};
\node[n] (e) [right of=g] {$\epsilon : ?$};
\node (h1) [right of=e] {$h = 1$};
\node (h0) [above of=h1] {$h = 0$};
\node (h2) [below of=h1] {$h = 2$};
...
\path (ae) edge [bend right,red,dashed] (a)
      (ae) edge [red,dashed] (b)
      (ae) edge [red,dashed] (g);
...
\centering
\scalebox{1.2}{
\begin{tikzpicture}[-latex,auto,node distance=1.0cm]
\tikzstyle{n} = [draw,shape=rectangle]
\tikzstyle{op} = [draw,shape=circle, inner sep=-0.04cm, outer sep=0]
\tikzstyle{wrap} = [draw,gray,dashed,shape=rectangle, minimum height=1.8cm,
minimum width=4.4cm]
\tikzset{every text node part/.style={align=center}}
\node[n] (st1) {$\tau_1$};
\node[n] (st2) [right of=st1] {$\tau_2$};
\node (stdots) [right of=st2] {$\cdots$};
\node[n] (stN) [right of=stdots] {$\tau_N$};
\node[op] (stcomb) [below right=0.7cm and 0.0cm of st2] {\Large$+$};
\node (stmlab) [above right=0.3cm and -0.5cm of st1] {Short-term Model};
\node (stbias) [below=0.0cm of stcomb] {$b_{\mathrm{int}}$};
\node[wrap] (stwrap) [below=-0.75cm of stmlab] {};

\node[n] (lt1) [right = 1.5cm of stN] {$\tau_1$};
\node[n] (lt2) [right of=lt1] {$\tau_2$};
\node (ltdots) [right of=lt2] {$\cdots$};
\node[n] (ltN) [right of=ltdots] {$\tau_N$};
\node (ltlab) [above right=0.3cm and -0.5cm of lt1] {Long-term Model};
\node[op] (ltcomb) [below right=0.7cm and 0.0cm of lt2] {\Large$+$};
\node (ltbias) [below=0.0cm of ltcomb] {$b_{\mathrm{int}}$};
\node[wrap] (ltwrap) [below=-0.75cm of ltlab] {};

\node[op] (comb) [below right=2.0cm and 2.3cm of stcomb] {\Large$+$};
\node (extbias) [above=0.0cm of comb] {$b_{\mathrm{ext}}$};
\node (pred) [below=0.8cm of comb] {Final Prediction};

\path (st1) edge (stcomb) 
  (st2) edge (stcomb) 
  (stN) edge (stcomb)
  (lt1) edge (ltcomb)
  (lt2) edge (ltcomb)
...
\centering
\scalebox{1.0}{
\begin{tikzpicture}[-latex,auto,node distance=3.5cm]
\tikzstyle{n} = [draw,shape=rectangle]
\tikzstyle{cell} = [draw,shape=rectangle, minimum width=1.8cm, minimum
height=1cm]
\tikzstyle{op} = [draw,shape=circle, inner sep=-0.03cm, outer sep=0]
\tikzstyle{wrap} = [draw,gray,dashed,shape=rectangle, minimum height=1.8cm,
minimum width=4.4cm]
\tikzset{every text node part/.style={align=center}}

\node[cell] (a11) {$A_1^1$};
\node[cell] (a21) [right of=a11] {$A_2^1$};
\node (rdots1) [right of=a21] {$\cdots$};
\node[cell] (aT1) [right of=rdots1] {$A_T^1$};

\node[cell] (a12) [above =1.5cm of a11] {$A_1^2$};
\node[cell] (a22) [right of=a12] {$A_2^2$};
\node (rdots2) [right of=a22] {$\cdots$};
\node[cell] (aT2) [right of=rdots2] {$A_T^2$};

\node (vdots1) [above = 1.5cm of a12] {$\vdots$};
\node (vdots2) [above = 1.5cm of a22] {$\vdots$};
\node (vdotsT) [above = 1.5cm of aT2] {$\vdots$};

\node[cell] (a1L) [above = 1.5cm of vdots1] {$A_1^L$};
\node[cell] (a2L) [right of=a1L] {$A_2^L$};
\node (rdotsL) [right of=a2L] {$\cdots$};
\node[cell] (aTL) [right of=rdotsL] {$A_T^L$};

\node[cell] (s1) [above = 1.0cm of a1L] {softmax};
\node[cell] (s2) [right of=s1] {softmax};
\node[cell] (sT) [above = 1.0cm of aTL] {softmax};

\node (p1) [above =0.5cm of s1] {$p(y_1)$};
\node (p2) [above =0.5cm of s2] {$p(y_2)$};
\node (pT) [above =0.5cm of sT] {$p(y_T)$};

\node (ini1) [left = 1.0cm of a11] {$\vect{h}_{\mathrm{init}}^1$};
\node (ini2) [left = 1.0cm of a12] {$\vect{h}_{\mathrm{init}}^2$};
\node (iniL) [left = 1.0cm of a1L] {$\vect{h}_{\mathrm{init}}^L$};

\node (x1) [below = 1.0cm of a11] {$\vect{h}_1^0 = E(x_1)$};
\node (x2) [below = 1.0cm of a21] {$\vect{h}_2^0 = E(x_2)$};
\node (xT) [below = 1.0cm of aT1] {$\vect{h}_T^0 = E(x_T)$};

\path (a11) edge node [near start] {$\vect{h}_1^1$} (a21);
\path (a21) edge node [near start] {$\vect{h}_2^1$} (rdots1);
\path (rdots1) edge node [near start] {$\vect{h}_{T-1}^1$} (aT1);

\path (a12) edge node [near start] {$\vect{h}_1^2$} (a22);
\path (a22) edge node [near start] {$\vect{h}_2^2$} (rdots2);
\path (rdots2) edge node [near start] {$\vect{h}_{T-1}^2$} (aT2);

\path (a1L) edge node [near start] {$\vect{h}_1^L$} (a2L);
\path (a2L) edge node [near start] {$\vect{h}_2^L$} (rdotsL);
\path (rdotsL) edge node [near start] {$\vect{h}_{T-1}^L$} (aTL);

\path (a11) edge node [near start] {$\vect{h}_1^1$} (a12);
\path (a21) edge node [near start] {$\vect{h}_2^1$} (a22);
\path (aT1) edge node [near start] {$\vect{h}_T^1$} (aT2);

\path (a12) edge node [near start] {$\vect{h}_1^2$} (vdots1);
\path (a22) edge node [near start] {$\vect{h}_2^2$} (vdots2);
\path (aT2) edge node [near start] {$\vect{h}_T^2$} (vdotsT);

\path (vdots1) edge node [near start] {$\vect{h}_1^{L-1}$} (a1L);
\path (vdots2) edge node [near start] {$\vect{h}_2^{L-1}$} (a2L);
\path (vdotsT) edge node [near start] {$\vect{h}_T^{L-1}$} (aTL);

\path (a1L) edge node {$\vect{h}_1^L$} (s1);
\path (a2L) edge node {$\vect{h}_2^L$} (s2);
\path (aTL) edge node {$\vect{h}_T^L$} (sT);
...
$\delta_{\mathrm{LSTM}}$ is given by equations
...
\scalebox{1.2}{
\begin{tikzpicture}[-latex,auto,node distance=1.5cm]
\tikzstyle{n} = [draw,shape=rectangle, minimum width=1cm, minimum height=0.6cm]
\tikzstyle{m} = [inner sep = -0.01mm, outer sep = -0.01mm]
\tikzstyle{op} = [draw,shape=circle, inner sep=0, outer sep=0]
\tikzstyle{wrap} = [draw,shape=rectangle, minimum height=3.5cm,
minimum width=8.0cm]
\tikzset{every text node part/.style={align=center}}

\node[n] (fgate) {$\sigma$};
\node[n] (igate) [right of=fgate] {$\sigma$};
\node[n] (feature) [right of=igate] {$\tanh$};
\node[n] (ogate) [right of=feature] {$\sigma$};
\node[op] (omul) [right of=ogate] {$\times$};
\node[n] (otanh) [above=0.5cm of omul] {$\tanh$};

\node[m] (i2fg) [below=0.5cm of fgate] {};
\node[m] (i2ig) [below=0.5cm of igate] {};
\node[m] (i2feat) [below=0.5cm of feature] {};
\node[m] (i2og) [below=0.5cm of ogate] {};
\node[m] (c2out) [above=0.55cm of otanh] {};
\node[m] (og2hout) [right=2.0cm of i2og] {};

\node[op] (dmul) [above=0.5cm of feature] {$\times$};
\node[op] (dadd) [above=0.5cm of dmul] {$+$};
\node[op] (fmul) [left=2.6cm of dadd] {$\times$};

\node[wrap] (wrap) [above=-1.5cm of feature] {};

\node (hin) [left=1.5cm of i2fg] {$[\vect{h}_{t-1}^l, \vect{h}_t^{l-1}]$};
\node (cin) [left=1.5cm of fmul] {$\vect{c}_{t-1}^l$};
\node (cout) [right=2.0cm of c2out] {$\vect{c}_t^l$};
\node (hout) [right=1.5cm of og2hout] {$\vect{h}_t^l$};

\path (i2fg) edge (fgate)
      (i2ig) edge (igate)
      (i2feat) edge (feature)
      (i2og) edge (ogate)
      (fgate) edge node {$\vect{f}$} (fmul)
      (igate) edge [bend left=30] node {$\vect{i}$} (dmul)
      (feature) edge node {$\vect{d}$} (dmul)
      (dmul) edge (dadd)
      (ogate) edge node {$\vect{o}$} (omul)
      (c2out) edge (otanh)
      (otanh) edge[-] (omul)
      (omul) edge [bend right=20] (hout.west);

\path (hin) edge[-] (i2og)
      (cin) edge[-,thick] (fmul)
      (fmul) edge[-,thick] (dadd)
      (dadd) edge[thick] (cout);
...
\node[n] (ctx) {Context\\$(e_1,\ldots,e_k)$};
\node (model) [right = 1.0cm of ctx] {\textbf{Generative}\\\textbf{Model}};
\node[n] (MVS) [above= 0.3cm of model,line width=0.5mm,blue] {\textbf{MVS}};
\node[n] (RNN) [below= 0.3cm of model,gray] {RNN};
\node[n] (dist) [right= 1.0cm of model] {Prediction\\$p(e' | e_1, \ldots, e_k)$ };
\node[n] (sample) [right= 0.8cm of dist] {Sample\\$e_{k+1}$};

\path (ctx) edge[thick,blue] (MVS) 
      (ctx) edge[gray] (RNN)
      (MVS) edge[thick,blue] (dist)
      (RNN) edge[gray] (dist)
      (dist) edge (sample);
...
\node[n] (root-1) {$() : 8$};
\node[n] (b-1) [below of=root-1] {$\beta : 1$};
\node[n] (bg-1) [below of=b-1] {$\beta\gamma : 1$};
\node[n] (bgd-1) [below of=bg-1] {$\beta\gamma\delta : 1$};
\node[n] (a-1) [left=2.0cm of b-1] {$\alpha : 3$};
\node[n] (ag-1) [thick,red,below of=a-1] {$\alpha\gamma : 1$};
\node[n] (aga-1) [thick,red,below of=ag-1] {$\alpha\gamma\alpha  : 1$};
\node[n] (ab-1) [left=0.5cm of ag-1]  {$\alpha\beta : 1$};
\node[n] (ad-1) [right of= ag-1] {$\alpha\delta : 1$};
\node[n] (abg-1) [below of=ab-1] {$\alpha\beta\gamma : 1$};
\node[n] (g-1) [right= 0.6cm of b-1] {$\gamma : 2$};
\node[n] (ga-1) [below of=g-1] {$\gamma\alpha : 1$};
\node[n] (gad-1) [below of=ga-1] {$\gamma\alpha\delta : 1$};
\node[n] (gd-1) [right=0.5cm of ga-1] {$\gamma\delta : 1$};
\node[n] (gda-1) [below of=gd-1] {$\gamma\delta\alpha : 1$};
\node[n] (d-1) [right of=g-1] {$\delta : 1$};
\node (dots-1) [below right of=d-1] {$\cdots$};
\node (info-1) [right=0.0cm of dots-1] {
  \begin{minipage}{3cm}
    \small
    \begin{align*}
      \mathit{Dead} &= \varnothing \\
      \mathit{Alive} &= \set{\alpha,\beta,\gamma,\delta} \\
      \mathit{Known} &= \set{\alpha} \\
      \mathit{Novel} &= \set{\beta,\gamma,\delta} \\
      s &= 1 \\
      p_\epsilon &= 1/2
    \end{align*}
  \end{minipage}
...
\node[n] (root-2) [below= 1.0cm of bgd-1] {$() : 8$};
\node[n] (b-2) [below of=root-2] {$\beta : 1$};
\node[n] (bg-2) [below of=b-2] {$\beta\gamma : 1$};
\node[n] (bgd-2) [below of=bg-2] {$\beta\gamma\delta : 1$};
\node[n] (a-2) [left=2.0cm of b-2] {$\alpha : 3$};
\node[n] (ag-2) [below of=a-2] {$\alpha\gamma : 1$};
\node[n] (aga-2) [below of=ag-2] {$\alpha\gamma\alpha  : 1$};
\node[n] (ab-2) [left=0.5cm of ag-2]  {$\alpha\beta : 1$};
\node[n] (ad-2) [right of= ag-2] {$\alpha\delta : 1$};
\node[n] (abg-2) [below of=ab-2] {$\alpha\beta\gamma : 1$};
\node[n] (g-2) [thick,red,right= 0.6cm of b-2] {$\gamma : 2$};
\node[n] (ga-2) [thick,red,below of=g-2] {$\gamma\alpha : 1$};
\node[n] (gad-2) [below of=ga-2] {$\gamma\alpha\delta : 1$};
\node[n] (gd-2) [thick,red,right=0.5cm of ga-2] {$\gamma\delta : 1$};
\node[n] (gda-2) [below of=gd-2] {$\gamma\delta\alpha : 1$};
\node[n] (d-2) [right of=g-2] {$\delta : 1$};
\node (dots-2) [below right of=d-2] {$\cdots$};
\node (info-2) [right=0.0cm of dots-2] {
  \begin{minipage}{3cm}
    \small
    \begin{align*}
      \mathit{Dead} &= \set{\alpha} \\
      \mathit{Alive} &= \set{\beta,\gamma,\delta} \\
      \mathit{Known} &= \set{\alpha,\delta} \\
      \mathit{Novel} &= \set{\beta,\gamma} \\
      s &= 1 \\
      p_\epsilon &= 1/2
    \end{align*}
  \end{minipage}
...
% step 1->2
\path (bgd-1) edge [dashed] 
      node[left]{escape} 
      node {$\frac{1}{2}\cdot p(\beta|\gamma)$} (root-2);
...
\node[n] (root-3) [thick,red,below= 1.0cm of bgd-2] {$() : 8$};
\node[n] (b-3) [thick,red,below of=root-3] {$\beta : 1$};
\node[n] (bg-3) [below of=b-3] {$\beta\gamma : 1$};
\node[n] (bgd-3) [below of=bg-3] {$\beta\gamma\delta : 1$};
\node[n] (a-3) [thick,red,left=2.0cm of b-3] {$\alpha : 3$};
\node[n] (ag-3) [below of=a-3] {$\alpha\gamma : 1$};
\node[n] (aga-3) [below of=ag-3] {$\alpha\gamma\alpha  : 1$};
\node[n] (ab-3) [left=0.5cm of ag-3]  {$\alpha\beta : 1$};
\node[n] (ad-3) [right of= ag-3] {$\alpha\delta : 1$};
\node[n] (abg-3) [below of=ab-3] {$\alpha\beta\gamma : 1$};
\node[n] (g-3) [thick,red,right= 0.6cm of b-3] {$\gamma : 2$};
\node[n] (ga-3) [below of=g-3] {$\gamma\alpha : 1$};
\node[n] (gad-3) [below of=ga-3] {$\gamma\alpha\delta : 1$};
\node[n] (gd-3) [right=0.5cm of ga-3] {$\gamma\delta : 1$};
\node[n] (gda-3) [below of=gd-3] {$\gamma\delta\alpha : 1$};
\node[n] (d-3) [thick,red,right of=g-3] {$\delta : 1$};
\node (dots-3) [below right of=d-3] {$\cdots$};
\node (info-3) [right=0.0cm of dots-3] {
  \begin{minipage}{3cm}
    \small
    \begin{align*}
      \mathit{Dead} &= \set{\alpha,\delta} \\
      \mathit{Alive} &= \set{\beta,\gamma} \\
      \mathit{Known} &= \set{\alpha,\beta,\gamma} \\
      \mathit{Novel} &= \varnothing \\
      s &= 3 \\
      p(\beta) &= 1/3
    \end{align*}
  \end{minipage}
};
\node (return) [below of=bgd-3] {return
$\frac{1}{2}\cdot\frac{1}{2}\cdot\frac{1}{3}$};

% step 2->3
\path (bgd-2) edge [dashed] 
      node[left]{escape} 
      node {$\frac{1}{2}\cdot\frac{1}{2}\cdot p(\beta)$} (root-3);
...
\node[n] (ctx) {Context\\$(e_1,\ldots,e_k)$};
\node (model) [right = 1.0cm of ctx] {\textbf{Generative}\\\textbf{Model}};
\node[n] (MVS) [above= 0.3cm of model,gray] {MVS};
\node[n] (RNN) [below= 0.3cm of model,blue,line width=0.5mm] {\textbf{RNN}};
\node[n] (dist) [right= 1.0cm of model] {Prediction\\$p(e' | e_1, \ldots, e_k)$ };
\node[n] (sample) [right= 0.8cm of dist] {Sample\\$e_{k+1}$};

\path (ctx) edge [gray] (MVS) 
      (ctx) edge[thick,blue] (RNN)
      (MVS) edge [gray] (dist)
      (RNN) edge[thick,blue] (dist)
      (dist) edge (sample);
...
optimisation process of \sref{sec:mvs-hyperparams}, while the RNN
