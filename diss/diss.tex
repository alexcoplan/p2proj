...
MVS literature (e.g.\ \cite{conklin1995viewpoints}) was
...
Pearce \cite{pearce2004improved} introduces a new method for distribtuion
...
\cite{Goodfellow-et-al-2016}. However, the negative effects of this problem on
training, illustrated in Figure~\ref{fig:exploding-grad}, can largely be
mitigated by \emph{clipping} the the gradients to some maximum norm during
training.
...
experience an exponential slow-up in learning such dependencies. For this
reason, learning long-term dependencies with basic RNNs is considered
intractable.
...
enabling LSTM networks to learn long-term dependencies over many timesteps. The
idea is to maintain additional state known as the \emph{cell state} which flows
through the network with only minor \emph{linear}, \emph{pointwise}
interactions.
...
Since LSTMs maintian an additional cell state $\vect{c}_t^l$, the transition
...
Note that, as before, parameters are shared across timesteps but not between
layers: layer superscripts are excluded from the LSTM parameters for clarity.

The purpose of the cell state $\vect{c}_t^l$ is to act as a memory.  The LSTM
can be understood intuitively as using this cell state following three
...
Inspection of relevant corpora indicated that writing a parser for an
...
The other major choice of library was that of a library to assist in the
implementation of the RNN. I wanted a library where the level of abstraction
offers considerable flexibility, but mundane tasks such as calculating the
gradients to perform the backwards pass through the network during training were
handled automatically. Google's TensorFlow \cite{abadi2016tensorflow}, with its
...
\begin{itemize}
  \item A correspondence between the \texttt{C++} type system and MVS formalism
    provides the basis for an API that should be intelligble by both
...
taken to ensure the normalisation of the distributions predicted by PPM. The
...
types}, in particular enabling a viewpoint linking pitch and duration.
...
viewpoints as inherently symmetric objects in that $\tau_1 \otimes \tau_2$
...
viewpoint, one simply marginalises the joint distribution: a viewpoint $\tau_1
\rightarrow \tau_2$ predicts a distribution over $[\tau_1] \times [\tau_2]$ and
marginalises to obtain a distribution over $[\tau_2]$.
...
require implementing a specialised linked viewpoint for this combination of types
that performs the lifting and reification for \emph{seqint}.
...
algorithm. Specifically, we implement a forward stepwise selection algorithm to
achieve this. For \ref{itm:hbar} and \ref{itm:bias-params} we use a grid search,
...
A heuristic approach to choosing the hyperparameters, would be to run the
