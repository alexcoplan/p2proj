...
\caption{Keyboard illustrating one octave with note names annotated using SPN}
...
In the following chapter, we introduce the supporting theory and background
for the chosen techniques, as well as justifying the decisions made in
order to refine the proposal ahead of the implementation.
Chapter~\ref{chap:impl} then details the implementation itself. In
Chapter~\ref{chap:eval}, the implemented models are evaluated, with the
conclusions presented in Chapter~\ref{chap:conc}.
...
motivates this choice. Secondarily, as a by-product of implementing a general
framework for multiple viewpoints, it is my intention that an open source
project that may be of use for further work can be released.
...
research or implementation in this area. All of the code written for this
project, aside from established libraries, was written from scratch.
...
objective evaluation and evaluation survey in detail.
...
An internal format for melody based on a widely-used representation in the
multiple viewpoint system literature (e.g.\ \cite{conklin1995viewpoints}) was
decided on. Musical time is quantised with a quantum of
$1/16$\textsuperscript{th} notes (semiquavers), since this is the smallest note
...
where $p$ is the MIDI pitch number of $n$, $o$ is the onset time, and $d$ the
...
using the assumption of (\ref{eq:cond-indep}): a multiple viewpoint system and
recurrent neural network. To generate a candidate $\vect{x}'$ from these models,
we make use of (iterative) random walk. The next two sections introduce these
models, with a focus on the underlying theory and decisions made prior to
implementation.
...
data structure storing sequences in $[\tau]^*$ together with a contextual
inference procedure exploiting the Markov assumption (\ref{eq:markov-asmptn}) to
predict distributions over $[\tau]$. 
...
Finally, so that we can favour distributions with lower $\hat{H}$ (lower
uncertainty), we introduce an exponential bias $b \in \mathbb{R}_0^+$:
$$ w_i = \hat{H}(i)^{-b}. $$
...
In a MVS, this is achieved explicitly by using separate \emph{long-term} and
\emph{short-term} models. The long-term model is trained on the entire corpus,
and the short-term model solely on the sequence being predicted or generated.
Figure~\ref{fig:mvs-arch} illustrates this architecture: viewpoint combination
is indicated by $\oplus$, and is performed as per Section~\ref{sec:vp-comb}.
...
training can largely be mitigated by \emph{clipping} the norm of the gradient to
some maximum value during training.
...
Here, we follow of Goodfellow et al.\ \cite{Goodfellow-et-al-2016} and refer to
...
There are in fact many variants of LSTM: we follow Zaremba et al.\
...
Before implementing either model, the corpus needed to be
...
output the corresponding JSON, as well as performing additional processing such
as \emph{anacrusis} detection. By preprocessing the corpus, we factor out this
...
\begin{itemize}
  \item A correspondence between the \texttt{C++} type system and MVS
...
at compile time, achieving better safety guarantees: in particular, they
eliminate much of the need for dynamic memory allocation, a cause of many common
runtime issues. As we shall see, specific viewpoints are implemented by
...
first tackle the task of constructing the underlying data structure, and then
turn to the implementation of PPM.
...
which I derived from the high-level descriptions given in the literature.
...
was found to suffer from numerical instability. This was detected through the
presence of extremely high or infinite cross-entropy on the test set. 
...
needed. After an initial investigation of the pseudo-random number generators
(PRNGs) available in the \texttt{C++11} standard library, I found several other
PRNGs that were shown to be superior in both performance and statistical
...
seeded using the system's source of randomness, falling back to high-resolution
uptime when this is not available.
...
music. This motivates the use of linked viewpoints. I first implemented a
generic \texttt{BasicLinkedViewpoint} class which implements a linked viewpoint
over arbitrary \emph{basic types}, in particular enabling a viewpoint linking
pitch and duration.
...
derived types implemented thus far rely on specialised code for lifting and
reification. To implement $\emph{seqint} \rightarrow \emph{duration}$, we would
need to implement a specific linked viewpoint for this combination of types that
...
We can handle \ref{itm:set-vps} with an automatic viewpoint selection algorithm.
Specifically, we implement a forward stepwise selection algorithm to achieve
this. For \ref{itm:hbar} and \ref{itm:bias-params} we use a grid search,
illustrated in Figure~\ref{fig:grid-search}. 
...
found to improve performance considerably.
...
errors occur following a lengthy computation. 
...
which was found to be highly effective at catching errors statically and
improving the overall efficiency of development in Python.
...
To guide the discussion of the core RNN implementation, we consdier each
...
Section~\ref{sec:rnn-train}, I clip the norm of gradients in the network to some
...
successfully prevented overfitting in a recurrent network and led to improved
...
training data. It is no surprise, then, that artifically translating or
...
instead artifically transpose the training data into many new keys (under
...
in ornamentation and tonality, the data had to be partitioned based on the
underlying melody. To achieve this, the mapping between chorales and
...
test loss. For the MVS, this was obtained using the hyperparameter optimisation
...
compositions to be a primary evaluation criterion of interest.
